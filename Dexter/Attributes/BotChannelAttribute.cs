    ...
    public class BotChannelAttribute : PreconditionAttribute
    {
                ...
                CommandInfo commandInfo, IServiceProvider services)
        {
            ...
            return Task.FromResult(
                services.GetRequiredService<BotConfiguration>().BotChannels.Contains(context.Channel.Id) || context.Channel.GetType() == typeof(SocketDMChannel) ?
                    PreconditionResult.FromSuccess() :
                    PreconditionResult.FromError($"Heya! You're not permitted to use the command `{commandInfo.Name}` " +
                    $"in the channel `#{context.Channel}`. Please use a designated bot channel instead <3"));
        }
