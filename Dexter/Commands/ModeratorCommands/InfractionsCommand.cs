        ...
        public EmbedBuilder[] GetWarnings(ulong User, ulong RunBy, string Mention, string Username, bool ShowIssuer) {
            ...
            for (int Index = 0; Index < Infractions.Length; Index++) {
                ...
                EmbedFieldBuilder Field = new EmbedFieldBuilder()
                    .WithName($"{(Infraction.InfrationTime == 0 ? "Warning" : $"{TimeSpan.FromSeconds(Infraction.InfrationTime).Humanize()} Mute")} {Index + 1} (ID {Infraction.InfractionID}), -{Infraction.PointCost} {(Infraction.PointCost == 1 ? "Point" : "Points")}.")
